...
export default function UploadRescueScreen() {
  ...
  const fetchLocation = useCallback(async () => {
    if (Platform.OS === 'android') {
      ...
      if (granted !== 'granted') {
        Alert.alert(
          'Permission Denied',
          'Location is needed to create an accurate report.',
        );
        return;
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!netInfo.isConnected) {
      Alert.alert(
        'No Internet',
        'An internet connection is required to submit a report.',
      );
      return;
    }
    if (!location) {
      Alert.alert(
        'Location Required',
        'Please wait for location to be fetched or enable permissions.',
      );
      return;
    }
